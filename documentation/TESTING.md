...
- During the testing stages of the project, I was having issues with removing items from the shopping bag. Issue was occuring when you add the same Whole Bean product with more than one type of grind, and trying to remove one of the grinds. Doing this was resulting in removing all the items with the same Coffee Bean type, instead of removing the specified girnd type. After further testing, and going over the code, I've realized there was a logic error. I had the `data-coffe-grind` in the anchor data, but in JS I was trying to get `grind` instead of `coffee-grind`, and I was passing it as `grind` while Python view was expecting `coffee-grind`. After adjusting the code, this issue was fixed, and I was successfully able to edit coffee grind types on the shopping page. [see commit](https://github.com/yigitaksoy/Rebel-Beans/commit/2a94b82d65d432210d49cec24226bffa432dbc70)

- During the late testing stages of the project, I found a bug in the code where user can exceed the restricted amount of items in their bag by manually editing the quantity, eventhough the quanitity indicators are disabled. The issue was coming from the `quantity_input_script` which I took from the Boutique Ado project. After further testing, I manually added an additional `EventListener` for manual changes in the quantity inputs, and sets the maximum amount to 99 if it exceeds. After updating the code issue was fixed, and users were restricted to override the item quantity, and maximum quantity was set to 99. [see commit](https://github.com/yigitaksoy/Rebel-Beans/commit/dd8d39033a2c55fa87081fb6825fb3663ada6a16)

- During the late testing stages of the project, I found a bug on my checkout success page. Issue was eventhough there was a successful checkout, `grand_total` wasn't being updated and showed as €0 in Production mode, but working without any problems in Development. After further testing, I found that this issue was being caused by Django itself, and was occuring on Django versions 3.2.4 - 3.2.7. To fix this issue `weak=False` was added into checkout signal receiver. After the following change, checkout signals started to operate as they should, and the issue was resolved. [see comit](https://github.com/yigitaksoy/Rebel-Beans/commit/e27eac0973f8a78a45345c78d9b945596c8321fd) 
